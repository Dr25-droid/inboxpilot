"""pilot_gmail.py — InboxPilot with real Gmail API integration.

Full production assistant connected to Gmail:
- Reads emails from your actual inbox via Gmail API
- Triage, human approval, and persistent memory — same as pilot.py
...
from inboxpilot.tools.gmail.prompt_templates import GMAIL_TOOLS_PROMPT
from inboxpilot.tools.gmail.gmail_tools import mark_as_read
...
from inboxpilot.utils import parse_gmail, format_for_display, format_gmail_markdown
...
def triage_router(state: State, store: BaseStore) -> Command[Literal["triage_interrupt_handler", "response_agent", "__end__"]]:
    """Classify Gmail email using triage preferences from memory."""
    ...
    system_prompt = triage_system_prompt.format(background=get_background(), triage_instructions=triage_instructions)
...
def triage_interrupt_handler(state: State, store: BaseStore) -> Command[Literal["response_agent", "__end__"]]:
    ...
    request = {
        "action_request": {"action": f"InboxPilot: {state['classification_decision']}", "args": {}},
...
def llm_call(state: State, store: BaseStore):
    """LLM decides which tool to call, reading preferences from memory."""
    ...
    for tool_call in state["messages"][-1].tool_calls:
        ...
        if response["type"] == "accept":
            ...
            if tool_call["name"] == "send_email_tool":
                ...
                goto = END
                update_memory(store, ("inboxpilot", "triage_preferences"), state["messages"] + result + [{"role": "user", "content": f"The user ignored the email draft. Update triage preferences. {MEMORY_UPDATE_INSTRUCTIONS_REINFORCEMENT}"}])
            elif tool_call["name"] == "schedule_meeting_tool":
                result.append({"role": "tool", "content": "User ignored this calendar meeting draft. Ignore this email and end the workflow.", "tool_call_id": tool_call["id"]})
                goto = END
                update_memory(store, ("inboxpilot", "triage_preferences"), state["messages"] + result + [{"role": "user", "content": f"The user ignored the calendar meeting draft. Update triage preferences. {MEMORY_UPDATE_INSTRUCTIONS_REINFORCEMENT}"}])
            elif tool_call["name"] == "Question":
                result.append({"role": "tool", "content": "User ignored this question. Ignore this email and end the workflow.", "tool_call_id": tool_call["id"]})
                goto = END
                update_memory(store, ("inboxpilot", "triage_preferences"), state["messages"] + result + [{"role": "user", "content": f"The user ignored the Question. Update triage preferences. {MEMORY_UPDATE_INSTRUCTIONS_REINFORCEMENT}"}])
            else:
                raise ValueError(f"Invalid tool call: {tool_call['name']}")
...
def should_continue(state: State, store: BaseStore) -> Literal["interrupt_handler", "mark_as_read_node"]:
    """Route to human review or mark-as-read when Done is called."""
...
def mark_as_read_node(state: State):
    """Mark the processed Gmail message as read."""
    email_input = state["email_input"]
    author, to, subject, email_thread, email_id = parse_gmail(email_input)
...
agent_builder.add_conditional_edges(
    ...
    {"interrupt_handler": "interrupt_handler", "mark_as_read_node": "mark_as_read_node"},
)
