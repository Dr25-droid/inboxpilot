"""pilot_review.py — Email assistant with human approval, without persistent memory.

Triage emails, then pause for human review before sending any reply or calendar invite.
"""
...
from inboxpilot.tools.prompt_templates import HITL_TOOLS_PROMPT
from inboxpilot.prompts import (
    triage_system_prompt, triage_user_prompt, agent_system_prompt_hitl,
    get_background, default_triage_instructions,
    default_response_preferences, default_cal_preferences,
)
from inboxpilot.schemas import State, RouterSchema, StateInput
...
def triage_router(state: State) -> Command[Literal["triage_interrupt_handler", "response_agent", "__end__"]]:
    """Classify email as respond, notify, or ignore."""
...
def triage_interrupt_handler(state: State) -> Command[Literal["response_agent", "__end__"]]:
    """Show NOTIFY emails to the user and let them decide to respond or dismiss."""
    author, to, subject, email_thread = parse_email(state["email_input"])
    email_markdown = format_email_markdown(subject, author, to, email_thread)
...
def interrupt_handler(state: State) -> Command[Literal["llm_call", "__end__"]]:
    """Pause for human review of draft emails, meetings, and questions."""
    result = []
    goto = "llm_call"
    ...
    for tool_call in state["messages"][-1].tool_calls:
        ...
        if response["type"] == "accept":
            ...
            if tool_call["name"] in ("write_email", "schedule_meeting", "Question"):
                msg = {
                    "write_email": "User ignored this email draft. Ignore this email and end the workflow.",
                    "schedule_meeting": "User ignored this calendar meeting draft. Ignore this email and end the workflow.",
                    "Question": "User ignored this question. Ignore this email and end the workflow.",
                }[tool_call["name"]]
                result.append({"role": "tool", "content": msg, "tool_call_id": tool_call["id"]})
                goto = END
            else:
                raise ValueError(f"Invalid tool call: {tool_call['name']}")
